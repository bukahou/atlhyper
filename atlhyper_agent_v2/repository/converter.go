...
//
// converter.go - K8s 原生类型到 model_v2 类型的转换函数
//
// 本文件提供所有资源的类型转换函数，将 K8s API 返回的原生类型
// 转换为项目统一定义的 model_v2 类型。
//
// 设计原则:
//   - 只提取业务需要的字段，不完全复制 K8s 结构
//   - 填充 CommonMeta 的关联字段 (NodeName, OwnerKind 等)
//   - 敏感信息 (Secret/ConfigMap 值) 只存 key 不存 value
//
// 转换函数命名规范:
//   - ConvertXxx: 将 K8s 类型转换为 model_v2 类型
