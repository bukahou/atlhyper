// // external/ingest/receivers/podlist_ingest.go
// package receivers
...
// // HandlePodListSnapshotIngest 处理 /ingest/pods/snapshot
// // - 仅接受 Source = "pod_list_snapshot" 的 Envelope
// // - 校验必要字段：Version / ClusterID / TimestampMs / Payload
// // - 成功返回 204，无响应体
// func HandlePodListSnapshotIngest(c *gin.Context) {
// 	// Pod 列表可能较大，给更高的 body 限制（可按需调整）
// 	c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, 16<<20) // 16MB
...
import (
	...
	"NeuroController/model"
	"NeuroController/model/envelope"
	ziputil "NeuroController/utils" // 根目录通用 gzip 工具
