...
func computeClusterUsageSeriesAll(
	...
	// 从接口拿“窗口内全部快照”（按节点、时间升序）
	snaps, err := Snapshot.GetAllNodeMetricsByCluster(ctx, clusterID)
	...
	earliestSet := false
	for _, arr := range snaps { // arr: []model.NodeMetricsSnapshot（值切片）
		if len(arr) == 0 {
			continue
		}
		// 保险：再按时间升序一遍（若接口已保证升序，也无妨）
...
// 	snaps := Snapshot.GetAllNodeMetricsByCluster()
