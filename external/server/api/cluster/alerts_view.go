...
import (
	"NeuroController/external/interfaces/events"
	"context"
	"sort"
	"strings"
	"time"
...
func BuildAlertsViewData(ctx context.Context, clusterID string,  recentLimit int) (dayTotal int, recent []RecentAlertRow, trends AlertTrendsSeries, err error) {
	// 1) 拉取最近 1 天日志
	logs, err := events.GetRecentEventLogs(clusterID, 1)
	if err != nil {
