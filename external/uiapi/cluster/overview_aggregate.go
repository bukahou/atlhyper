...
// - 30 分钟 CPU/内存/温度折线
// - 各节点 CPU/内存使用率
func BuildClusterOverviewAggregated(ctx context.Context) (*AnalysisPayload, error) {
	...
	cpuSeries, memSeries, tempSeries := computeClusterUsageSeriesAll(time.Time{}, 0)

	// 3) Alerts：一天窗口（总数/轻量表格/趋势）
	dayTotal, recentAlerts, alertTrends, err := BuildAlertsViewData(ctx, 10)
	if err != nil {
		return nil, err
	}
