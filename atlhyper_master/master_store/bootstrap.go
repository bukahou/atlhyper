...
// - 配置：使用默认参数 → TTL=24小时、最大 5 万条、清理周期 1 分钟
...
func Bootstrap() {
	...
	// 2) 启动定期清理（TTL + 容量）
	StartTTLJanitor(defaultTTL, defaultMaxItems, defaultInterval, metricsTTL)

	log.Printf("master_store bootstrap ok (TTL=%s, cap=%d, interval=%s, metricsTTL=%s)",
		defaultTTL, defaultMaxItems, defaultInterval, metricsTTL)
}
