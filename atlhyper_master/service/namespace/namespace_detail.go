...
func BuildNamespaceDetail(ctx context.Context, clusterID, name string) (*ui.NamespaceDetailDTO, error) {
...
func fromModelToDetail(ns mod.Namespace) ui.NamespaceDetailDTO {
	out := ui.NamespaceDetailDTO{
		// 基本
	...
	// 配额
	if len(ns.Quotas) > 0 {
		out.Quotas = make([]ui.NamespaceQuotaDTO, 0, len(ns.Quotas))
		for _, q := range ns.Quotas {
			out.Quotas = append(out.Quotas, ui.NamespaceQuotaDTO{
				Name:   q.Name,
				Scopes: q.Scopes,
				Hard:   q.Hard,
				Used:   q.Used,
	...
	if len(ns.LimitRanges) > 0 {
		out.LimitRanges = make([]ui.NamespaceLimitRangeDTO, 0, len(ns.LimitRanges))
		for _, lr := range ns.LimitRanges {
			itemDTOs := make([]ui.NamespaceLimitRangeItem, 0, len(lr.Items))
			for _, it := range lr.Items {
				itemDTOs = append(itemDTOs, ui.NamespaceLimitRangeItem{
					Type:                 it.Type,
					Max:                  it.Max,
					Min:                  it.Min,
					Default:              it.Default,
			...
			out.LimitRanges = append(out.LimitRanges, ui.NamespaceLimitRangeDTO{
				Name:  lr.Name,
				Items: itemDTOs,
			})
		}
	...
	// 指标
	if ns.Metrics != nil {
		out.Metrics = &ui.NamespaceMetricsDTO{
			CPU: ui.NamespaceResourceAgg{
			...
			Memory: ui.NamespaceResourceAgg{
