...
func BuildIngressOverview(ctx context.Context, clusterID string) (*ui.IngressOverviewDTO, error) {
...
func fromModelToOverview(items []mod.Ingress) ui.IngressOverviewDTO {
	rows := make([]ui.IngressRowSimple, 0, 64)
	...
	for _, in := range items {
		...
		if len(in.Spec.Rules) == 0 {
			...
			rows = append(rows, ui.IngressRowSimple{
	...
	return ui.IngressOverviewDTO{
		Cards: ui.IngressOverviewCards{
