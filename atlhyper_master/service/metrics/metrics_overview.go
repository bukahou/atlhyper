...
func BuildNodeMetricsOverview(ctx context.Context, clusterID string) (*ui.MetricsOverviewDTO, error) {
	now := time.Now().UTC()
	...
	series, err := repository.GetClusterMetricsRange(ctx, clusterID, since, now)
	if err != nil {
		return nil, err
	}
	...
	rows := make([]ui.MetricsNodeRow, 0, len(latestByNode))
	...
	dto := &ui.MetricsOverviewDTO{
		Cards: ui.MetricsOverviewCards{
