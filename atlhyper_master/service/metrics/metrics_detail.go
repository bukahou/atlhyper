...
func BuildNodeMetricsDetail(ctx context.Context, clusterID, node string) (*ui.MetricsDetailDTO, error) {
	const minutes = 15
	...
	series, err := repository.GetClusterMetricsRange(ctx, clusterID, since, until)
	if err != nil {
		return nil, err
	}
	...
	// 3) 无数据兜底
	if len(points) == 0 {
		dto := &ui.MetricsDetailDTO{
			Node:   node,
			Latest: ui.MetricsNodeRow{},
			Series: ui.MetricsSeries{
				At:     []time.Time{},
				CPUPct: []float64{},
				MemPct: []float64{},
				TempC:  []float64{},
