...
func BuildDeploymentDetail(ctx context.Context, clusterID, namespace, name string) (*dto.DeploymentDetailDTO, error) {
...
func fromModelToDetail(d mod.Deployment) dto.DeploymentDetailDTO {
	out := dto.DeploymentDetailDTO{
		// summary
		Name:        d.Summary.Name,
		...
		// template
		Template: dto.DeploymentTemplateDTO{
		...
		// status
		Status: dto.DeploymentStatusDTO{
	...
	// spec
	out.Spec = dto.DeploymentSpecDTO{
	...
	if len(d.Status.Conditions) > 0 {
		out.Conditions = make([]dto.DeploymentCondDTO, 0, len(d.Status.Conditions))
		for _, c := range d.Status.Conditions {
			out.Conditions = append(out.Conditions, dto.DeploymentCondDTO{
				Type:               c.Type,
				Status:             c.Status,
				Reason:             c.Reason,
				Message:            c.Message,
	...
	// rollout
	if d.Rollout != nil {
		out.Rollout = &dto.DeploymentRolloutDTO{
	...
	if len(d.ReplicaSets) > 0 {
		out.ReplicaSets = make([]dto.ReplicaSetBriefDTO, 0, len(d.ReplicaSets))
		for _, rs := range d.ReplicaSets {
			out.ReplicaSets = append(out.ReplicaSets, dto.ReplicaSetBriefDTO{
				Name:      rs.Name,
