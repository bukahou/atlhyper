// atlhyper_master/service/namespace/namespace_detail.go
package namespace
...
func BuildNamespaceDetail(ctx context.Context, clusterID, name string) (*dto.NamespaceDetailDTO, error) {
...
func fromModelToDetail(ns mod.Namespace) dto.NamespaceDetailDTO {
	out := dto.NamespaceDetailDTO{
		// 基本
	...
	// 配额
	if len(ns.Quotas) > 0 {
		out.Quotas = make([]dto.NamespaceQuotaDTO, 0, len(ns.Quotas))
		for _, q := range ns.Quotas {
			out.Quotas = append(out.Quotas, dto.NamespaceQuotaDTO{
				Name:   q.Name,
				Scopes: q.Scopes,
				Hard:   q.Hard,
				Used:   q.Used,
	...
	if len(ns.LimitRanges) > 0 {
		out.LimitRanges = make([]dto.NamespaceLimitRangeDTO, 0, len(ns.LimitRanges))
		for _, lr := range ns.LimitRanges {
			itemDTOs := make([]dto.NamespaceLimitRangeItem, 0, len(lr.Items))
			for _, it := range lr.Items {
				itemDTOs = append(itemDTOs, dto.NamespaceLimitRangeItem{
					Type:                 it.Type,
					Max:                  it.Max,
					Min:                  it.Min,
					Default:              it.Default,
			...
			out.LimitRanges = append(out.LimitRanges, dto.NamespaceLimitRangeDTO{
				Name:  lr.Name,
				Items: itemDTOs,
			})
		}
	...
	// 指标
	if ns.Metrics != nil {
		out.Metrics = &dto.NamespaceMetricsDTO{
			CPU: dto.NamespaceResourceAgg{
			...
			Memory: dto.NamespaceResourceAgg{
