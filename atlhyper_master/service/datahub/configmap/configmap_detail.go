...
func BuildConfigMapListFullByNamespace(ctx context.Context, clusterID, namespace string) ([]dto.ConfigMapDTO, error) {
	if namespace == "" {
		return nil, fmt.Errorf("namespace required")
	}

	// 如果后端已有按 ns 拉取的接口，优先使用：
	// list, err := repository.GetConfigMapListByNamespaceLatest(ctx, clusterID, namespace)
	// 这里先复用全量 + 过滤
	list, err := repository.Mem.GetConfigMapListLatest(ctx, clusterID)
	if err != nil {
	...
	dtos := make([]dto.ConfigMapDTO, 0, len(filtered))
...
func fromModel(cm mod.ConfigMap, withData bool) dto.ConfigMapDTO {
	out := dto.ConfigMapDTO{
	...
	if withData {
		...
		if n := len(cm.Data); n > 0 {
			out.Data = make([]dto.ConfigMapDataEntry, 0, n)
			for _, d := range cm.Data {
				out.Data = append(out.Data, dto.ConfigMapDataEntry{
					Key:       d.Key,
					Size:      d.Size,
					Preview:   d.Preview,
		...
		if m := len(cm.Binary); m > 0 {
			out.Binary = make([]dto.ConfigMapBinaryEntry, 0, m)
			for _, b := range cm.Binary {
				out.Binary = append(out.Binary, dto.ConfigMapBinaryEntry{
					Key:  b.Key,
					Size: b.Size,
				})
			}
