...
func BuildServiceOverview(ctx context.Context, clusterID string) (*dto.ServiceOverviewDTO, error) {
...
func fromModelToOverview(svcs []mod.Service) dto.ServiceOverviewDTO {
	rows := make([]dto.ServiceRowSimple, 0, len(svcs))
	...
	for _, s := range svcs {
		...
		rows = append(rows, dto.ServiceRowSimple{
			Name:      s.Summary.Name,
	...
	return dto.ServiceOverviewDTO{
		Cards: dto.ServiceCards{
			TotalServices:    total,
