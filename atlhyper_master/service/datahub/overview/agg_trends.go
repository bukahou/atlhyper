...
// 资源趋势：按分钟取峰值并记录峰值节点
func buildResourceUsageTrends(snaps []collect.NodeMetricsSnapshot, since, until time.Time) []dto.ResourceTrendPoint {
    // 统一用 UTC 做桶边界
    start := ceilToBucket(since.UTC(), time.Minute) // ✅ 起点向上取整
    end   := until.UTC()                            // 区间上界仍为开区间

    points := make(map[time.Time]*dto.ResourceTrendPoint)
    ...
    for _, s := range snaps {
        ...
        p, ok := points[bucket]
        if !ok {
            p = &dto.ResourceTrendPoint{At: bucket}
            points[bucket] = p
        }
        // 峰值统计（CPU / Mem / 温度）……
    ...
    out := make([]dto.ResourceTrendPoint, 0, n)
    ...
    for t := start; t.Before(end); t = t.Add(time.Minute) {
        if p, ok := points[t]; ok {
            ...
            out = append(out, dto.ResourceTrendPoint{At: t})
        }
    }
    return out
}
...
func buildPeakStats(snaps []collect.NodeMetricsSnapshot) dto.TrendPeakStats {
	if len(snaps) == 0 {
		return dto.TrendPeakStats{HasData: false}
	}
...
func buildAlertHourlyFromEvents(events []transport.LogEvent, since, until time.Time) []dto.AlertHourlyPoint {
	// 先桶化
	...
	// 再按时间顺序补齐 24 个桶
	out := make([]dto.AlertHourlyPoint, 0, int(until.Sub(since)/time.Hour))
	for t := floorToBucket(since, time.Hour); t.Before(until); t = t.Add(time.Hour) {
		a := bm[t]
		if a == nil {
			out = append(out, dto.AlertHourlyPoint{At: t})
		} else {
			out = append(out, dto.AlertHourlyPoint{
				At:       t,
				Critical: a.c,
				Warning:  a.w,
				Info:     a.i,
