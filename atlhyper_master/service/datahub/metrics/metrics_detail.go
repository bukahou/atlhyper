// atlhyper_master/service/metrics/metrics_detail.go
package metrics
...
func BuildNodeMetricsDetail(ctx context.Context, clusterID, node string) (*dto.MetricsDetailDTO, error) {
	const minutes = 15
	...
	series, err := repository.Mem.GetClusterMetricsRange(ctx, clusterID, since, until)
	if err != nil {
		return nil, err
	}
	...
	// 3) 无数据兜底
	if len(points) == 0 {
		dto := &dto.MetricsDetailDTO{
			Node:   node,
			Latest: dto.MetricsNodeRow{},
			Series: dto.MetricsSeries{
				At:     []time.Time{},
				CPUPct: []float64{},
				MemPct: []float64{},
				TempC:  []float64{},
