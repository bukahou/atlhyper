...
// 3. 整体内存使用率 (UsageCard)
func buildNodeUsagesAndTotals(nodes []k8s.Node) ([]ui.OverviewNodeUsageRow, ui.UsageCard, ui.UsageCard) {
	out := make([]ui.OverviewNodeUsageRow, 0, len(nodes))
	...
	for _, n := range nodes {
		...
		// ---- append row ----
		out = append(out, ui.OverviewNodeUsageRow{
			Node:     n.Summary.Name,
			CPUUsage: cpuPct,
			MemUsage: memPct,
		})
	...
	return out, ui.UsageCard{Percent: cpuTotalPct}, ui.UsageCard{Percent: memTotalPct}
}
