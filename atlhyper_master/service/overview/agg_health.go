...
func buildClusterHealth(pods []k8s.Pod, nodes []k8s.Node) ui.ClusterHealthCard {
	// Pod Ready 率
...
func buildNodeReady(nodes []k8s.Node) ui.NodeReadyCard {
	total := len(nodes)
	ready := 0
	for _, n := range nodes {
		if strings.ToLower(n.Summary.Ready) == "true" {
	...
	return ui.NodeReadyCard{Total: total, Ready: ready, Percent: pct}
}
