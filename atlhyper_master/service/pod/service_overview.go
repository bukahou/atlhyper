...
//         items = append(items, ui.PodOverviewItem{
...
func BuildPodOverview(ctx context.Context, clusterID string) (*ui.PodOverviewDTO, error) {
    pods, err := repository.GetPodListLatest(ctx, clusterID)
    if err != nil {
        return nil, err
    }
    ...
    var running, pending, failed, unknown int
    items := make([]ui.PodOverviewItem, 0, len(pods))
    ...
    return &ui.PodOverviewDTO{
        Cards: ui.PodCards{Running: running, Pending: pending, Failed: failed, Unknown: unknown},
        Pods:  items,
    }, nil
}
