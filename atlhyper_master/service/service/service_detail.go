...
func GetServiceDetail(ctx context.Context, clusterID, namespace, name string) (*ui.ServiceDetailDTO, error) {
...
func fromModelToDetail(s mod.Service) ui.ServiceDetailDTO {
	dto := ui.ServiceDetailDTO{
		// 基本
		Name:      s.Summary.Name,
		...
		// 选择器 & 端口
		Selector: s.Selector,
		Ports:    make([]ui.ServicePortDTO, 0, len(s.Ports)),
	...
	for _, p := range s.Ports {
		dto.Ports = append(dto.Ports, ui.ServicePortDTO{
			Name:        p.Name,
			Protocol:    p.Protocol,
			Port:        p.Port,
