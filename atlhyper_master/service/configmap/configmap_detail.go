...
func BuildConfigMapListFullByNamespace(ctx context.Context, clusterID, namespace string) ([]ui.ConfigMapDTO, error) {
	if namespace == "" {
	...
	// 这里先复用全量 + 过滤
	list, err := repository.GetConfigMapListLatest(ctx, clusterID)
	if err != nil {
	...
	dtos := make([]ui.ConfigMapDTO, 0, len(filtered))
...
func fromModel(cm mod.ConfigMap, withData bool) ui.ConfigMapDTO {
	dto := ui.ConfigMapDTO{
	...
	if withData {
		...
		if n := len(cm.Data); n > 0 {
			dto.Data = make([]ui.ConfigMapDataEntry, 0, n)
			for _, d := range cm.Data {
				dto.Data = append(dto.Data, ui.ConfigMapDataEntry{
					Key:       d.Key,
					Size:      d.Size,
					Preview:   d.Preview,
		...
		if m := len(cm.Binary); m > 0 {
			dto.Binary = make([]ui.ConfigMapBinaryEntry, 0, m)
			for _, b := range cm.Binary {
				dto.Binary = append(dto.Binary, ui.ConfigMapBinaryEntry{
					Key:  b.Key,
					Size: b.Size,
				})
			}
