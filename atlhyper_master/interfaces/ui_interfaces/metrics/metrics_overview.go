...
func BuildNodeMetricsOverview(ctx context.Context, clusterID string) (*NodeMetricsOverviewDTO, error) {
	now := time.Now().UTC()
	since := now.Add(-2 * time.Minute)

	// 如果你有 “最新快照” 接口，优先用：
	// snaps, err := datasource.GetClusterMetricsLatest(ctx, clusterID)
	// 下面是通用兜底：从近2分钟范围内取最后一条作为“最新”
	series, err := datasource.GetClusterMetricsRange(ctx, clusterID, since, now)
	if err != nil {
		return nil, err
	}
	...
	rows := make([]NodeMetricsRow, 0, len(latestByNode))
