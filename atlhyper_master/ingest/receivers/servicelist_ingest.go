// external/ingest/receivers/servicelist_ingest.go
package receivers
...
// HandleServiceListSnapshotIngest 处理 /ingest/services/snapshot
...
func HandleServiceListSnapshotIngest(c *gin.Context) {
	...
	// 5) 基本校验
	if env.Source != SourceServiceListSnapshot {
		c.JSON(http.StatusBadRequest, gin.H{"message": "source 非法：仅支持 service_list_snapshot"})
		return
	}
