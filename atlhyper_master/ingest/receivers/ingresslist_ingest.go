// external/ingest/receivers/ingresslist_ingest.go
package receivers
...
// HandleIngressListSnapshotIngest 处理 /ingest/ingresses/snapshot
// - 兼容压缩与未压缩：支持 Content-Encoding:gzip，且自动嗅探魔数
// - 校验必要字段：Version / ClusterID / TimestampMs / Payload
// - 成功返回 204，无响应体
func HandleIngressListSnapshotIngest(c *gin.Context) {
	...
	// 5) 基本校验
	if env.Source != SourceIngressListSnapshot {
		c.JSON(http.StatusBadRequest, gin.H{"message": "source 非法：仅支持 ingress_list_snapshot"})
		return
	}
