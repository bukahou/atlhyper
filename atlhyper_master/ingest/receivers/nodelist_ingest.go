// external/ingest/receivers/nodelist_ingest.go
package receivers
...
// HandleNodeListSnapshotIngest 处理 /ingest/nodes/snapshot
...
func HandleNodeListSnapshotIngest(c *gin.Context) {
	...
	// 5) 基本校验
	if env.Source != SourceNodeListSnapshot {
		c.JSON(http.StatusBadRequest, gin.H{"message": "source 非法：仅支持 node_list_snapshot"})
		return
	}
