// external/ingest/receivers/configmaplist_ingest.go
package receivers
...
// HandleConfigMapListSnapshotIngest 处理 /ingest/configmaps/snapshot
...
func HandleConfigMapListSnapshotIngest(c *gin.Context) {
	...
	// 5) 基本校验
	if env.Source != SourceConfigMapListSnapshot {
		c.JSON(http.StatusBadRequest, gin.H{"message": "source 非法：仅支持 configmap_list_snapshot"})
		return
	}
