// external/ingest/receivers/namespacelist_ingest.go
package receivers
...
import (
	...
	ziputil "AtlHyper/utils" // 通用 gzip 工具：压缩/自动解压/透传

	"github.com/gin-gonic/gin"
)

// 仅接收 namespace 列表快照
const SourceNamespaceListSnapshot = model.SourceNamespaceListSnapshot

// HandleNamespaceListSnapshotIngest 处理 /ingest/namespaces/snapshot
// - 兼容压缩与未压缩：支持 Content-Encoding:gzip，且自动嗅探魔数
// - 校验必要字段：Version / ClusterID / TimestampMs / Payload
// - 成功返回 204，无响应体
func HandleNamespaceListSnapshotIngest(c *gin.Context) {
	...
	// 5) 基本校验
	if env.Source != SourceNamespaceListSnapshot {
		c.JSON(http.StatusBadRequest, gin.H{"message": "source 非法：仅支持 namespace_list_snapshot"})
		return
	}
