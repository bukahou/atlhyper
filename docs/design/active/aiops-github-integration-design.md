...
> 状态: future | 创建: 2026-03-08 | 更新: 2026-03-10
...
│  GitHub     │     │  设置: /settings/deploy│    │  展示: AIOps 事件详情  │
│  Actions    │     │                      │     │                      │
└──────┬──────┘     └──────────┬───────────┘     └──────────┬───────────┘
       │                       │                            │
       │                 无依赖关系                    无依赖关系
...
type DeployConfig struct {
    RepoURL     string // git@github.com:user/Config.git
    Branch      string // "main"
...
CREATE TABLE deploy_config (
    ...
    repo_url      TEXT NOT NULL,           -- Config 仓库地址
    branch        TEXT DEFAULT 'main',
    deploy_path   TEXT NOT NULL,           -- kustomization.yaml 所在子目录
    auth_type     TEXT DEFAULT 'ssh_key',  -- ssh_key / token
    auth_secret   TEXT DEFAULT '',         -- 加密存储
...
| **Web /settings/deploy** | Config 仓库配置、调谐参数、认证方式 | — |
| **Web /admin/deploy** | 查看同步状态、部署历史、触发回滚 | 不触发主动部署 |

### 2.8 Agent 新增 Command
...
│  Config 仓库                                            │
│  ────────────────────────────────────────────────────── │
│  仓库地址:  [git@github.com:user/Config.git       ]    │
│  分支:      [main                                 ]    │
...
| `gateway/handler/admin/deploy.go` | 部署管理 API |
| `gateway/handler/settings/deploy.go` | 部署配置 API |
| `atlhyper_web/src/app/admin/deploy/` | 部署管理页面 |
| `atlhyper_web/src/app/settings/deploy/` | 部署配置页面 |
...
| 项目 | 说明 |
|------|------|
| **Config 仓库认证** | SSH Key 或 Token 加密存储（common/crypto），不明文 |
| **Token 安全** | GitHub App 私钥 / Client Secret 加密存储 |
