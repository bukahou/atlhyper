...
- 事件上下文构建在 `Enhancer.Summarize()` 内，同步执行
...
### 4.4 Enhancer System Prompt 更新

在 `enhancer.go` 的 system prompt 中新增指引：
...
atlhyper_master_v2/
├── ai/
│   ├── prompts.go              ← [修改] toolsJSON 新增 4 个工具定义 + rolePrompt 新增工具使用说明
...
├── aiops/
│   └── ai/
│       ├── prompts.go          ← [修改] SummarizePrompt() 拼接新增 OTel 上下文字段
│       ├── analysis.go         ← [修改] buildAnalysisSystemPrompt() 新增调查策略指引
│       ├── context_builder.go  ← [修改] IncidentContext 新增 3 个字段 + buildOTelContext()
│       └── enhancer.go         ← [修改] Summarize() 传入 OTelSnapshot 参数
└── ...（其他目录不变）

atlhyper_agent_v2/              ← 零改动（所有 Command Handler 已存在）
...
| 文件 | 变更类型 | 变更内容 | Phase |
|------|---------|---------|-------|
| `ai/prompts.go` | **修改** | ① `toolsJSON` 追加 4 个工具 JSON Schema ② `rolePrompt` 新增 `[可观测性查询工具]` + `[工具组合使用建议]` | P1-P2 |
| `ai/tool.go` | **修改** | 新增 `truncateToolResult()`、`truncateLogBodies()`、`buildEntityKey()`、`simplifyEntityDetail()` | P1-P2 |
| `master.go` | **修改** | `// 9.2 注册 AIOps Tool` 区块新增 4 个 `aiService.RegisterTool()` 调用 | P1-P2 |
| `aiops/ai/analysis.go` | **修改** | `buildAnalysisSystemPrompt()` 新增调查策略和信号关联指引 | P1-P2 |
| `aiops/ai/prompts.go` | **修改** | `SummarizePrompt()` 拼接 `ctx.RecentErrorTraces`/`RecentErrorLogs`/`SLOContext` | P3 |
| `aiops/ai/context_builder.go` | **修改** | `IncidentContext` 新增 3 个字段 + `buildOTelContext()` 函数 | P3 |
| `aiops/ai/enhancer.go` | **修改** | `Summarize()` 接受 `OTelSnapshot` 参数，传入 `buildOTelContext()` | P3 |

**总计：7 个文件修改，0 个新文件，Agent 端零改动。**
...
|------|-----------|------|-------------|
| **chat** | `ai/prompts.go` → `securityPrompt + rolePrompt` | 用户交互式对话 | ✅ 全部 8 个 |
| **background** | `aiops/ai/prompts.go` → `SystemPrompt` | 事件摘要（JSON 输出） | ❌ 无 Tool |
| **analysis** | `aiops/ai/analysis.go` → `buildAnalysisSystemPrompt()` | 高危事件深度调查 | ✅ 全部 8 个 |

### 10.2 chat `rolePrompt` 变更

在 `ai/prompts.go` 的 `rolePrompt` 中，`[AIOps 工具]` 区块需扩展为包含全部工具的说明：
...
### 10.3 analysis `buildAnalysisSystemPrompt()` 变更

在 `aiops/ai/analysis.go` 的深度分析提示词中，增加调查策略指引：
...
### 10.4 background `SystemPrompt` — 无变更

background 角色（`aiops/ai/prompts.go`）不使用 Tool，只做事件摘要的 JSON 结构化输出。
新增的 OTel 上下文数据（Traces/Logs/SLO）通过 `IncidentContext` 字段传入 user prompt，
...
| 文件 | 变更内容 | Phase |
|------|---------|-------|
| `ai/prompts.go` | `rolePrompt` 新增 `[可观测性查询工具]` + `[工具组合使用建议]` 区块 | P1-P2 |
| `aiops/ai/analysis.go` | `buildAnalysisSystemPrompt()` 新增调查策略和信号关联指引 | P1-P2 |
| `aiops/ai/prompts.go` | `SummarizePrompt()` 拼接新增的 OTel 上下文字段 | P3 |
