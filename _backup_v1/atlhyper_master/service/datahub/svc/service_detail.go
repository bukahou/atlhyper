// atlhyper_master/service/service/service_detail.go
package service
...
func GetServiceDetail(ctx context.Context, clusterID, namespace, name string) (*dto.ServiceDetailDTO, error) {
	list, err := repository.Mem.GetServiceListLatest(ctx, clusterID)
	if err != nil {
...
func fromModelToDetail(s mod.Service) dto.ServiceDetailDTO {
	out := dto.ServiceDetailDTO{
		// 基本
		Name:      s.Summary.Name,
		...
		// 选择器 & 端口
		Selector: s.Selector,
		Ports:    make([]dto.ServicePortDTO, 0, len(s.Ports)),
	...
	for _, p := range s.Ports {
		out.Ports = append(out.Ports, dto.ServicePortDTO{
			Name:        p.Name,
			Protocol:    p.Protocol,
			Port:        p.Port,
