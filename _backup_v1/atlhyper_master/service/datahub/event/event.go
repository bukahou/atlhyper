...
func BuildEventOverview(ctx context.Context, clusterID string, withinDays int) (*dto.EventOverviewDTO, error) {
	...
	// 2) 明细（把 RFC3339 传给底层）
	rows, err := repository.Event.GetSince(ctx, clusterID, since)
	if err != nil {
		return nil, err
	}
	...
	return &dto.EventOverviewDTO{Cards: cards, Rows: rows}, nil
}
