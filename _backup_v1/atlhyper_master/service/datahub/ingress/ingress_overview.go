...
func BuildIngressOverview(ctx context.Context, clusterID string) (*dto.IngressOverviewDTO, error) {
...
func fromModelToOverview(items []mod.Ingress) dto.IngressOverviewDTO {
	rows := make([]dto.IngressRowSimple, 0, 64)
	...
	for _, in := range items {
		...
		if len(in.Spec.Rules) == 0 {
			...
			rows = append(rows, dto.IngressRowSimple{
	...
	return dto.IngressOverviewDTO{
		Cards: dto.IngressOverviewCards{
