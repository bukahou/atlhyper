...
//     pods, err := repository.Mem.GetPodListLatest(ctx, clusterID)
//     if err != nil {
//         return nil, err
//     }
...
//         items = append(items, dto.PodOverviewItem{
...
func BuildPodOverview(ctx context.Context, clusterID string) (*dto.PodOverviewDTO, error) {
    pods, err := repository.Mem.GetPodListLatest(ctx, clusterID)
    if err != nil {
        return nil, err
    }
    ...
    var running, pending, failed, unknown int
    items := make([]dto.PodOverviewItem, 0, len(pods))
    ...
    return &dto.PodOverviewDTO{
        Cards: dto.PodCards{Running: running, Pending: pending, Failed: failed, Unknown: unknown},
        Pods:  items,
    }, nil
}
