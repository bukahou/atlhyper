...
// -----------------------------------------------------------------------------
// 一次循环完成：
// 1. 每个节点的 CPU/内存使用率 (OverviewNodeUsageRow)
// 2. 整体 CPU 使用率 (UsageCard)
// 3. 整体内存使用率 (UsageCard)
func buildNodeUsagesAndTotals(nodes []k8s.Node) ([]dto.OverviewNodeUsageRow, dto.UsageCard, dto.UsageCard) {
	out := make([]dto.OverviewNodeUsageRow, 0, len(nodes))
	...
	for _, n := range nodes {
		...
		// ---- append row ----
		out = append(out, dto.OverviewNodeUsageRow{
			Node:     n.Summary.Name,
			CPUUsage: cpuPct,
			MemUsage: memPct,
		})
	...
	return out, dto.UsageCard{Percent: cpuTotalPct}, dto.UsageCard{Percent: memTotalPct}
}
