// source/abnormal/abnormal_utils.go
// 异常检测工具函数
package abnormal
	...
	for _, cs := range pod.Status.ContainerStatuses {
		...
		if cs.State.Waiting != nil {
			...
			if reason, ok := PodAbnormalReasons[reasonCode]; ok {
				...
				exceptionID := GenerateExceptionID("Pod", pod.Name, pod.Namespace, reason.Code)
				if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
					return nil
				}
...
func GetNodeAbnormalReason(node corev1.Node) *NodeAbnormalReason {
	...
	for _, cond := range node.Status.Conditions {
		...
		if reason.Category == "Fatal" && (cond.Status == corev1.ConditionFalse || cond.Status == corev1.ConditionUnknown) {
			...
			exceptionID := GenerateExceptionID("Node", node.Name, "", reason.Code)
			if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
				return nil
			}
...
func GetEventAbnormalReason(event corev1.Event) *EventAbnormalReason {
	...
	if reason.Code == "Unhealthy" {
		exceptionID := GeneratePodInstanceExceptionID(
	...
	exceptionID := GenerateExceptionID("Event", event.InvolvedObject.Name, event.InvolvedObject.Namespace, reason.Code)
	if !ShouldProcessException(exceptionID, time.Now(), 2*time.Minute) {
		return nil
	}
...
func GetDeploymentAbnormalReason(deploy appsv1.Deployment) *DeploymentAbnormalReason {
	...
	if deploy.Status.UnavailableReplicas > 0 {
		...
		exceptionID := GenerateExceptionID("Deployment", name, namespace, reason.Code)
		if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
			return nil
		}
...
// 			exceptionID := GenerateExceptionID("Endpoints", ep.Name, ep.Namespace, rule.Code)
// 			if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
// 				return nil
// 			}
...
func GetEndpointAbnormalReason(slice *discoveryv1.EndpointSlice) *EndpointAbnormalReason {
	...
	for _, rule := range EndpointAbnormalRules {
		if rule.Check(slice) {
			exceptionID := GenerateExceptionID("EndpointSlice", slice.Name, slice.Namespace, rule.Code)
			if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
				return nil
			}
...
func GetServiceAbnormalReason(svc corev1.Service) *ServiceAbnormalReason {
	...
	if len(svc.Spec.Selector) == 0 {
		...
		exceptionID := GenerateExceptionID("Service", name, namespace, reason.Code)
		if !ShouldProcessException(exceptionID, now, 2*time.Minute) {
			return nil
		}
		return &reason
