...
func Init() {
	Global = &Store{
		data: make(map[string]collect.NodeMetricsSnapshot, 256),
	}
}
...
func Put(node string, snap *collect.NodeMetricsSnapshot) {
	if node == "" || snap == nil {
		return
	}
	ensureInit()
...
func PutSnapshot(snap *collect.NodeMetricsSnapshot) {
...
func GetAllLatestCopy() map[string]collect.NodeMetricsSnapshot {
	ensureInit()
	Global.mu.RLock()
	defer Global.mu.RUnlock()
	out := make(map[string]collect.NodeMetricsSnapshot, len(Global.data))
