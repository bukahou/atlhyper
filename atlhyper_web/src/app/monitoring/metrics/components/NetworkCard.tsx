      ...
      {(totalErrors > 0 || totalDropped > 0) && (
        <div className="hidden sm:block mt-4 pt-4 border-t border-[var(--border-color)]">
          <div className="flex items-center gap-2 text-xs text-muted">
            ...
            {totalErrors > 0 && (
              <span>{formatNumber(totalErrors)} {nm.network.errors}</span>
            )}
            {totalErrors > 0 && totalDropped > 0 && <span>·</span>}
            {totalDropped > 0 && (
              <span>{formatNumber(totalDropped)} {nm.network.dropped}</span>
            )}
            <span className="text-muted/60">({nm.network.sinceBootup})</span>
          </div>
        </div>
      )}
    </div>
