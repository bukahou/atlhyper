...
import { getServiceAccountList } from "@/datasource/cluster";
import type { ServiceAccountItem } from "@/api/cluster-resources";
...
import { ServiceAccountDetailModal } from "@/components/service-account";
...
export default function ServiceAccountPage() {
  ...
  // 详情弹窗状态
  const [selectedItem, setSelectedItem] = useState<ServiceAccountItem | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);

  const fetchData = useCallback(async () => {
    setError("");
    try {
      const res = await getServiceAccountList({ cluster_id: currentClusterId });
  ...
  const handleViewDetail = (item: ServiceAccountItem) => {
    ...
    <Layout>
      <div className="space-y-4">
        ...
        <FilterBar
          ...
          searchPlaceholder={t.common.search + " ServiceAccount..."}
        />
