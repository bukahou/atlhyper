...
// 概览 Tab
export function OverviewTab({ detail, t }: { detail: NodeDetail; t: Translations }) {
  const infoItems = [
...
// 资源 Tab
export function ResourcesTab({ detail, t }: { detail: NodeDetail; t: Translations }) {
  const ResourceBar = ({
    label,
    icon,
    used,
...
// 状态 Tab
export function ConditionsTab({ conditions, t }: { conditions: NodeCondition[]; t: Translations }) {
...
// 污点 Tab
export function TaintsTab({ taints, t }: { taints: NodeTaint[]; t: Translations }) {
  if (taints.length === 0) {
...
// 标签 Tab
export function LabelsTab({ labels, t }: { labels: Record<string, string>; t: Translations }) {
