...
- **L3**：APM + Logs 信号未接入推理管道，APM 拓扑边未加入依赖图（AIOps OTel 融合进行中）
- **L4**：缺异常模式库（历史事件聚类）、风险预测（基线趋势预判）、根因 Top-N + 置信度 + 证据链
- **L5**：Command 机制已支持 Pod 操作，但缺自动决策逻辑和策略引擎
...
| **AIOps**              | 基线学习, 异常关联, 风险评估 | Store (读), Database (读)     |
...
```go
// service/interfaces.go — 接口隔离（Query 按信号域拆分子接口）
type QueryK8s interface {         // K8s 资源查询
    ListClusters(...)
    GetPods(...)
}

type QueryObserve interface {     // 可观测性查询（Metrics/APM/Logs/SLO）
    GetMetricsSummary(...)
    GetTracesList(...)
    QueryLogs(...)
    GetSLOSummary(...)
...
type QueryOverview interface {    // 总览查询
    GetOverview(...)
}

type Query interface {            // 组合只读接口
    QueryK8s
    QueryObserve
    QueryOverview
}
...
// k8s.go, observe_metrics.go, observe_apm.go, observe_logs.go, observe_slo.go, overview.go
```
...
│   ├── ai/             #   AI Chat 组件
│   └── ...
├── datasource/         # 数据源层（API 调用 + mock 降级）
