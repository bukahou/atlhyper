...
- **五、运维规范** — Git 工作流、任务生命周期、设计文档工作流（功能驱动设计）、docs 目录结构
...
AtlHyper 是**面向 AI 时代的次世代 SRE 平台**，采用 Master-Agent 架构管理多集群 Kubernetes 环境。

**核心问题**：传统 SRE 的 Metrics / Logs / APM / SLO 是不同历史阶段提出的方案，天然割裂——数据孤岛、排查依赖人脑串联、知识不沉淀、监控与控制脱节。AtlHyper 的目标不是简单聚合数据，而是构建「系统运行认知模型」，让系统理解自己。

**五层能力模型**：

| 层级 | 能力 | 说明 | 状态 |
|------|------|------|------|
| **L1 统一语义层** | 数据关联 | 所有信号（Metrics/APM/Logs/SLO/K8s）映射到统一实体模型（Service/Pod/Node），消除数据孤岛 | ✅ |
| **L2 因果拓扑图** | 关系建模 | 实时依赖图（Pod→Node, Service→Pod, Service→Service），让「谁影响谁」可计算 | ✅ |
| **L3 推理引擎** | 异常推理 | 不是「CPU 高了」，而是「Service A 延迟上升因为依赖的 Service B 的 Pod 在 OOM」 | 🔄 |
| **L4 AI 增强** | 智能分析 | 基于结构化因果数据做模式识别、历史匹配、根因置信度输出 | 🔄 |
| **L5 自主控制** | 闭环自愈 | 观测→推理→决策→执行（自动隔离/重启/扩缩容/回滚） | ❌ |

**当前差距**：

- **L3**：AIOps 已实现 EMA 基线、依赖图、状态机、风险传播；APM 拓扑边已初步加入依赖图，但 Logs 信号尚未接入推理管道
- **L4**：已有事件模式查询（IncidentPattern），但缺风险预测（基线趋势预判）、根因 Top-N + 置信度 + 证据链
- **L5**：Command 机制已支持 19 种 K8s 操作，但缺自动决策逻辑和策略引擎

**技术栈**：Go (后端) + React/Next.js (前端) + SQLite (持久化) + ClickHouse (时序数据) + Redis (缓存/会话) + 内存 MQ (消息队列)
...
├── model_v3/               # 共用数据模型（按领域子包：cluster / agent / metrics / slo / command）
├── cmd/                    # 各组件入口 main.go
├── docs/                   # 文档（设计/任务/参考）
├── go.mod                  # Go 模块 (全项目共用)
└── CLAUDE.md               # 本文件
...
| `model_v3/`      | 共享数据模型（按领域子包划分，见下表）                   |

**model_v3 子包结构：**

| 子包 | 内容 |
|------|------|
| `model_v3/` (根) | `CommonMeta`, `ResourceRef`, `ParseCPU/ParseMemory` 等通用类型 |
| `model_v3/cluster/` | K8s 资源模型（Pod, Node, Deployment 等）+ ClusterSnapshot |
| `model_v3/agent/` | Agent 状态模型（AgentInfo, AgentStatus） |
| `model_v3/metrics/` | 节点指标模型（NodeMetrics, Summary） |
| `model_v3/slo/` | SLO 模型（IngressSLO, ServiceSLO, ServiceEdge） |
| `model_v3/command/` | 指令模型（Command, CommandResult） |
| `model_v3/apm/` | APM 模型（Trace, Span, APMService） |
| `model_v3/log/` | 日志模型（Entry, Summary, Facets） |
...
```
sdk/
├── interfaces.go              # 接口定义（K8sClient / ClickHouseClient）
├── types.go                   # 共用类型
└── impl/                      # 实现按职责分子目录
    ...
    │   ├── networking.go
    │   └── metrics.go
    └── clickhouse/            # ClickHouseClient 实现（查询 ClickHouse 时序数据）
        └── client.go
```
...
└── ch/         (ClickHouse 仓库)  └── command/     (指令服务)
    └── query/  (OTel 查询)            ├── command.go
                                       └── summary.go
```
...
| 原则           | 实践要求                                  |
| -------------- | ----------------------------------------- |
| **DRY**        | 跨项目共用的工具函数放 `common/`（如日志、加密、压缩）；跨项目共用的数据结构放 `model_v3/`（按领域子包：cluster、agent、metrics、slo、command） |
...
| 包 | 用途 | JSON Tag | 示例 |
|----|------|---------|------|
| `model_v3/` | Agent/Master 共享模型 | **camelCase** | `nodeName`, `createdAt`, `ownerKind` |
...
#### Convert 层（model_v3 → model 转换）

路径：`atlhyper_master_v2/model/convert/`

**职责：** 将 Agent 上报的 `model_v3` 领域模型转换为前端可直接渲染的 `model` 扁平结构。

**命名模式：**

| 函数类型 | 命名 | 示例 |
|---------|------|------|
| 列表项 | `ResourceItem(src)` | `PodItem(src *cluster.Pod) model.PodItem` |
| 批量转换 | `ResourceItems(src)` | `PodItems(src []cluster.Pod) []model.PodItem` |
| 详情 | `ResourceDetail(src)` | `PodDetail(src *cluster.Pod) model.PodDetail` |
...
| 职责 | 说明 | 示例 |
|------|------|------|
| **字段转换** | model_v3 → model 的映射、重命名、扁平化 | `CreatedAt time.Time` → `"createdAt": "2025-01-01T00:00:00Z"` |
...
**核心原则：SQLite 是嵌入式数据库，开发阶段直接删库重建，不做增量迁移。**

| 规则 | 说明 |
|------|------|
| **表结构变更** | 直接修改 `migrations.go` 中的 CREATE TABLE，不写 ALTER TABLE 增量迁移 |
| **种子数据变更** | 直接修改种子函数中的默认值，不考虑旧数据兼容 |
| **字段重命名/删除** | 直接改表定义，不保留旧列 |
| **重建方式** | 删除 SQLite 文件，重启服务自动重建 |
| **禁止** | 禁止为了兼容旧数据写"增量迁移跳过"逻辑（如 `duplicate column name` 的 WARN 日志） |

**理由**：AtlHyper 使用 SQLite 作为嵌入式配置数据库，数据量小、重建成本低。增量迁移增加代码复杂度且容易出错（如种子数据 `count > 0` 保护导致新默认值不生效）。
...
├── database/           # 数据库层 (SQLite 持久化)
├── service/            # 统一服务层
│   ├── interfaces.go   #   Service 接口 (所有方法)
│   ├── factory.go      #   工厂函数 (组合 query + operations)
│   ├── query/          #   查询实现 (读取)
│   ├── operations/     #   操作实现 (写入: command + admin)
│   └── sync/           #   数据同步 (Event 持久化)
├── gateway/            # 网关层 (HTTP API)
├── model/              # Master API 响应模型 + Convert 层
├── ai/                 # AI 模块 (LLM 集成 + 工具调用)
├── aiops/              # AIOps 模块 (基线/关联/风险/事件/状态机)
├── notifier/           # 通知模块 (触发/分发/丰富)
├── slo/                # SLO 路由更新器
├── deployer/           # GitOps 部署模块 (CD)
...
| **AIOps**              | 基线学习, 异常关联, 风险评估 | Store (读), Database (读写)   |
| **Notifier**           | 告警触发, 消息分发, 上下文丰富 | Store (读), Database (读写) |
| **SLO**                | SLO 路由动态更新            | Store (读)                    |
| **Deployer**           | GitOps 部署 (CD)            | GitHub, Database              |
| **Store (datahub/)**   | 数据存储 (快照/Agent/Event) | -                             |
| **MQ (mq/)**           | 消息队列 (指令队列)         | -                             |
| **Database**           | 持久化数据库 (SQLite)       | -                             |
...
```go
// service/interfaces.go — 接口隔离（Query 按功能域拆分子接口）

// 子接口（6 个）
type QueryK8s interface { ... }       // K8s 资源快照查询 (19 方法)
type QueryOTel interface { ... }      // OTel 快照/时间线查询 (2 方法)
type QuerySLO interface { ... }       // SLO 服务网格查询 (2 方法)
type QueryAIOps interface { ... }     // AIOps 查询与 AI 增强 (13 方法)
type QueryOverview interface { ... }  // 集群概览/Agent/事件/单资源 (11 方法)
type QueryAdmin interface { ... }     // 管理查询: 审计/指令/通知/设置/AI (18 方法)

// 写入接口
type OpsAdmin interface { ... }       // 管理写入: 通知/设置/AI Provider (9 方法)
type Ops interface {                  // 组合写入接口
    CreateCommand(...)
    OpsAdmin
}

// 组合接口
type Query interface {                // 组合所有只读子接口
    QueryK8s; QueryOTel; QuerySLO; QueryAIOps; QueryOverview; QueryAdmin
}
...
// service/query/ — 读取实现（按职责拆分文件）
// impl.go (QueryService struct), k8s.go, otel.go, slo.go, aiops.go, overview.go, admin.go, metrics_format.go
```

Gateway Handler 按需依赖最小接口：查询用 `service.Query`（或子接口），操作用 `service.Ops`。

### 2.8 初始化顺序

```go
1.  Store (datahub.NewStore)        // 数据存储 (内存/Redis)
2.  Bus (mq.NewCommandBus)          // 消息队列 (内存/Redis)
3.  Database                        // 持久化数据库 (SQLite)
4.  EventPersistService             // Event 持久化 (依赖 Store, Database)
5.  SLO RouteUpdater                // SLO 路由更新
6.  AIOps Engine                    // 风险评估引擎 (依赖 Store, Database)
7.  Processor                       // 数据处理 (依赖 Store, 回调: EventPersist + SLO + AIOps)
8.  operations.CommandService       // 指令操作 (依赖 Bus, Database)
9.  operations.AdminService         // 管理操作 (依赖 Database)
10. AI Service                      // LLM 集成 (依赖 Operations, Database)
11. AIOps Enricher                  // AI 增强 (依赖 AIOps Engine, AI Service)
12. query.QueryService              // 读取实现 (依赖 Store, Bus, Database, AIOps)
13. service.NewService(q, cmd, admin) // 组合为统一 Service
14. AgentSDK                        // Agent 通信 (依赖 Processor, Bus, Database)
15. AlertManager + Triggers         // 告警管理 (依赖 Store, Service, Database)
16. GitHub Client + Deployer        // GitOps (可选, 依赖 Database)
17. Gateway                         // Web API (依赖 Service)
```
...
持久化数据:
  → database/repo/ 新增 Repository 接口
  → database/sqlite/ 实现
```
...
├── deployer/                 # 示例：Deployer 模块
│   ├── interfaces.go         #   对外接口 (Deployer)
│   ├── service.go            #   实现
│   └── types.go              #   数据模型
└── github/                   # 示例：外部 API 封装
    ├── interfaces.go         #   对外接口 (Client)
    ├── api.go                #   API 实现
    └── types.go              #   数据模型
```
...
atlhyper_agent_v2/
├── sdk/
│   ├── interfaces.go              ← K8sClient / ClickHouseClient 接口
│   └── impl/
│       ├── k8s/                   ← K8sClient 实现（主动拉取 K8s API）
│       └── clickhouse/            ← ClickHouseClient 实现（查询 ClickHouse 时序数据）
├── repository/
│   ├── interfaces.go              ← 仓库接口（K8s / ClickHouse OTel）
│   ├── k8s/                       ← 21 个 K8s 资源仓库实现
│   └── ch/                        ← ClickHouse 查询仓库
│       └── query/                 ←   OTel 数据查询（trace/log/metrics/slo）
├── concentrator/                  ← 时序数据预聚合（可选，Ring Buffer）
├── service/
...
- **K8s SDK**：Repository 调用 K8sClient 主动拉取 K8s API Server
- **ClickHouse SDK**：Repository 调用 ClickHouseClient 查询 OTel 时序数据（Metrics/APM/Logs/SLO）
- **Concentrator**（可选）：对高频时序数据做 Ring Buffer 预聚合，减少网络传输
- 禁止 SDK 层直接依赖 Repository 层

### 3.5 Agent ↔ Master 通信

- 3 条数据通道：Snapshot（推送）、Command Poll（拉取）、Heartbeat（推送）
- 快照统一打包为 `ClusterSnapshot`（含 K8s 资源 + OTelSnapshot），单次 HTTP POST 发送
- 压缩/解压使用 `common.GzipBytes()` / `common.MaybeGunzipReaderAuto()`

### 3.6 初始化顺序

```go
1. SDK 层        — K8sClient（K8s 连接 + ClusterID 检测）
2. Gateway 层    — MasterGateway（Master 通信客户端）
3. Repository 层 — K8s repos（20 个）; 可选: ClickHouseClient + CH repos + Concentrator
4. Service 层    — SnapshotService（所有 repos）, CommandService（pod + generic repos）
5. Scheduler 层  — 调度器（编排 Snapshot/Command/Heartbeat 三条通道）
```
...
```
atlhyper_web/src/
├── api/                # API 模块（按资源/信号域拆分）
│   ├── request.ts      #   Axios 封装（拦截器、Token 注入）
│   ├── pod.ts          #   Pod 相关 API
│   ├── observe.ts      #   Observe API（Metrics/APM/Logs/SLO）
│   └── ...
...
│   ├── ai/             #   AI Chat 组件
│   └── ...
├── datasource/         # 数据源层（API 调用 + mock 降级，页面优先导入此层）
├── mock/               # Mock 数据（未登录时降级展示）
├── config/             # 前端配置（路由、主题等）
├── theme/              # 主题配置
├── i18n/               # 国际化
...
路径：`atlhyper_web/src/datasource/`

前端通过 datasource 层访问数据，而非直接调用 `api/`。datasource 封装了「真实 API 调用 + mock 数据降级」逻辑：

```
datasource/
├── slo.ts           # SLO 数据源（API + mock 降级）
├── mesh.ts          # Mesh 数据源
├── ...
```

- 每个 datasource 文件根据登录状态和数据源配置决定调用真实 API 还是返回 mock 数据
- 页面/组件只导入 datasource，不直接导入 api/
...
**Phase 划分原则：功能闭环优先，禁止半成品**

> 每个 Phase 完成后，用户必须能通过 UI 或 API 验证到一个**完整的功能闭环**。
> 禁止出现「写入了但查不到」「扣减了但没记录」「后端改了但前端看不到变化」的半成品状态。

检查方法：Phase 完成后问**「用户能在 UI 或 API 上看到什么完整的变化？」**——如果答案是「看不出来」或「数据不完整」，说明 Phase 边界划分有问题，需要合并或调整。

错误示例：
- Phase 1 做预算扣减，Phase 2 才补全报告元数据 → 扣了预算但报告里查不到具体消耗，半成品
- Phase 1 做后端 API，Phase 2 才做前端 → 如果 API 无法通过其他方式验证，也是半成品

正确做法：
- 按「用户可感知的完整功能」划分，而非按「技术层」划分
- 如果写入和查询是同一功能的两半，必须放在同一个 Phase

**Phase 验收检查清单（每个 Phase 完成时必须逐项执行）**

编译验证只能证明语法正确，不能证明功能完整。尤其是纯后端逻辑（无 UI 可观察），必须通过以下步骤兜底：

| # | 检查项 | 方法 | 目的 |
|---|--------|------|------|
| 1 | **编译通过** | `go build` / `next build` | 基本语法正确 |
| 2 | **设计文档逐项对账** | 逐条核对文件变更清单，确认每个文件的每个变更点都已实现 | 防止遗漏 |
| 3 | **数据通路完整性** | 追踪「数据从哪产生 → 经过哪些函数 → 最终存到哪 / 返回给谁」，确认链路无断点 | 防止半成品 |
| 4 | **接口签名核对** | 设计文档中定义的接口签名与实际代码一致 | 防止实现偏离设计 |

> 第 3 点是最容易遗漏的——一个功能可能涉及「产生 → 传递 → 存储 → 查询 → 展示」多个环节，
> 只完成其中几环而断在中间，编译不会报错，但功能是残缺的。
...
#### 设计文档编写方针：功能驱动设计（Feature-Driven Design）

**核心原则**：设计文档按**功能**分块，每块自包含完整的开发链路。禁止按技术模块（前端/后端/Agent）分散组织。
...
功能 X：功能名称
├── 用户故事（做什么、为什么）
├── API 定义（Method + Path + 请求 Body + 响应 JSON，每个端点完整示例）
├── 数据对照表（前端 Mock TypeScript ↔ 后端 Go struct ↔ DB 表，三层逐字段对齐）
├── 后端实现（涉及的文件和职责）
└── 前端实现（涉及的文件和职责，标注 [✅]/[待实现] 状态）
```
...
| 规则 | 说明 |
|------|------|
| **API 是前后端唯一契约** | 每个 API 同时定义请求/响应结构，前端 Mock 必须与后端响应 JSON 字段名和结构完全一致 |
| **三层数据对照** | 前端 TS 接口 ↔ 后端 Go struct（含 JSON tag）↔ DB 表字段，逐字段对齐防止遗漏 |
| **功能自包含** | 读完一个功能块就能直接开发，不需要跨章节拼凑信息 |
| **文件变更用树状图** | 按组件（Master/Agent/Web）分树，标注 `[新增]`/`[修改]`/`[✅]` 状态 |
| **架构放顶部，参考放附录** | 模块关系、技术选型放文档开头；CI 规范、约束风险等放附录 |
...
| Master API 参考   | `docs/static/reference/master-api-reference.md`  |
| Web API 参考      | `docs/static/reference/web-api-reference.md`     |
